...
def create_dataset(upscale_model, deblock_conf, deblock_model, deblur_conf, deblur_model, dataset_dir, images_dir, landmarks_txt, out_dir, num_gpu=0, num_threads=4, num_tasks=100):
    print('Loading image data from %s' % dataset_dir)
    with open(os.path.join(dataset_dir, landmarks_txt), 'rt') as file:
        files_landmarks = [(line.split()[0], [float(value) for value in line.split()[1:]]) for line in file.readlines()]
    ...
    from lapsrn_wgan import Net2x, Net4x, Net2x_last, Net4x_last
    torch.cuda.set_device(num_gpu)

    model8x = torch.load(upscale_model, map_location=lambda st, loc: st.cuda(num_gpu))['model']
    ...
    import caffe
    model_deblocking = caffe.Net(network_file=deblock_conf, phase=caffe.TEST, weights=deblock_model)
    model_debluring = caffe.Net(network_file=deblur_conf, phase=caffe.TEST, weights=deblur_model)
    ...
    def process_func(idx):
        # Load original image.
        name = os.path.splitext(os.path.basename(files[idx]))[0]
        savename = '{}-{:02d}.png'.format(name, (idx + 1) % 100)
        if any([os.path.isfile(os.path.join(dataset_dir, out_dir, nx + 'x', savename)) for nx in ['1', '2', '4', '8']]):
            return idx, None, None, savename
        ...
        if superres > 1:
            if superres == 8 or superres == 4 or superres == 2:
                ...
                if superres == 8:
                    model_deblocking.blobs['data'].reshape(1, 3, input_img_th.shape[1], input_img_th.shape[2])
                    model_deblocking.blobs['data'].data[...] = np.array([input_img_th])
                    model_deblocking.forward()
                    input_img_th = model_deblocking.blobs['d15'].data[0]

                    model_debluring.blobs['data'].reshape(1, 3, input_img_th.shape[1], input_img_th.shape[2])
                    model_debluring.blobs['data'].data[...] = np.array([input_img_th])
                    model_debluring.forward()
                    input_img_th = model_debluring.blobs['d15'].data[0]
                ...
                return idx, None, superres, savename
        ...
        return idx, img, superres, savename

    with ThreadPool(num_threads) as pool:
        print('%d / %d' % (0, len(files)))
        for idx, img, superres, savename in pool.process_items_concurrently(xrange(len(files)), process_func=process_func, max_items_in_flight=num_tasks):
            if img is not None:
                img = img.transpose(1, 2, 0)
                img = PIL.Image.fromarray(np.uint8(np.clip(np.round(img), 0, 255)), 'RGB')
                nx_dir = '1x' if superres <= 1 else '2x' if superres == 2 else '4x' if superres == 4 else '8x'
                img.save(os.path.join(dataset_dir, out_dir, nx_dir, savename))
                print('Process file %s: %d / %d' % (savename, idx + 1, len(files)))
            elif superres is not None:
                print('Skip file %s (%dx is not supported): %d / %d' % (savename, superres, idx + 1, len(files)))
            else:
                print('Skip existed file %s: %d / %d' % (savename, idx + 1, len(files)))
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_dataset', 'Create arbitrary high resolution dataset', 'create_dataset ~/dataset')
    p.add_argument(     'upscale_model',    help='2/4/8x upscaling pytorch model')
    p.add_argument(     'deblock_conf',     help='Deblocking caffe model config')
    p.add_argument(     'deblock_model',    help='Deblocking caffe model weights')
    p.add_argument(     'deblur_conf',      help='Deblurring caffe model config')
    p.add_argument(     'deblur_model',     help='Deblurring caffe model weights')
    p.add_argument(     'dataset_dir',      help='Directory to read image data from')
